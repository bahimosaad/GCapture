...
public class JFSComparison {
	...
	/**
	 * Inserts an element to the comparison table and starts the comparison
	 * algorithm recursively, if and only if the matched files are directories.
	 * At least one (source or target) file has to be not equal to null.
	 * 
	 * @param srcFile
	 *            A source file which may be null if no corresponding source
	 *            file exists, but a target file.
	 * @param tgtFile
	 *            A target file which may be null if no corresponding target
	 *            file exists, but a source file.
	 * @param parent
	 *            The parent element.
	 * @param isDirectory
	 *            Determines whether the files are directories.
	 */
	private final void add(JFSFile srcFile, JFSFile tgtFile, JFSElement parent,
			boolean isDirectory) {
		...
		JFSElement element = new JFSElement(srcFile, tgtFile, parent,
				isDirectory);
		JFSTable.getInstance().addElement(element);

		// Start algorithm recursivley, if the files are directories:
	...
	/**
	 * Compares two lists of files and writes the result to the comparison
	 * table. Both lists must be not equal to null.
	 * 
	 * @param srcFiles
	 ...
	 *            Determines whether the files are directories.
	 */
	private final void compareFiles(JFSFile[] srcFiles, JFSFile[] tgtFiles,
			JFSElement parent, boolean isDirectory) {
	...
	/**
	 * Compares two directories and writes the result to the comparison table.
	 * At least one (source or target) file has to be not equal to null.
	 * 
	 * @param srcDir
	 *            A source directory which may be null if no corresponding
	 *            source directory exists, but a target directory.
	 * @param tgtDir
	 *            A target directory which may be null if no corresponding
	 *            target directory exists, but a source directory.
	 * @param parent
	 *            The parent element.
	 */
	private final void compareDirectories(JFSFile srcDir, JFSFile tgtDir,
			JFSElement parent) {
	...
	/**
	 * Starts comparison for all directory pairs, computes the actions that have
	 * to be taken according to the chosen synchronization mode.
	 */
	public final void compare() {
		// Get all directory pairs:
		Vector<JFSDirectoryPair> pairs = JFSConfig.getInstance()
				.getDirectoryList();
		...
		for (JFSDirectoryPair pair : pairs) {
			...
			if (!progress.isCanceled()) {
				...
				if (root.isActive()) {
					compareDirectories(root.getSrcFile(), root.getTgtFile(),
							root);
				}
			}

            Vector<JFSCopyStatement> copyStatements = JFSTable.getInstance().getCopyStatements();
            System.out.println(" Cop Statements "+copyStatements.size());
			monitor.decrease();
			progress.fireUpdate();
		}
