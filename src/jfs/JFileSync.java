		...
		try {
			while (i < args.length) {
				if (args[i].equals("-debug")) {
					...
					i++;
					config.addDirectoryPair(new JFSDirectoryPair(args[i],
							args[i + 1]));
					i++;
					loadDefaultFile = false;
					...
					p.println(t.get("general.appName") + " "
