...
public class JFSQuestionPrint implements JFSQuestionOracle {
	...
	public final QuestionAnswer ask(JFSQuestion question) {
		...
		p.println("  " + t.get("syncQuestion.print.src") + " "
				+ JFSFormatter.adapt(e.getSrcFile().getPath(), 40) + " ("
				+ JFSFormatter.getLastModified(e.getSrcFile()) + ")");
		p.println("  " + t.get("syncQuestion.print.tgt") + " "
				+ JFSFormatter.adapt(e.getTgtFile().getPath(), 40) + " ("
