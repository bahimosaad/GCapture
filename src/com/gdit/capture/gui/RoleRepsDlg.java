 ...
 * @author Bahi
 */
public class RoleRepsDlg extends javax.swing.JDialog {

    /** Creates new form GroupsGUI */
    public RoleRepsDlg(Roles role, RolesPanel parent, boolean modal) {
    ...
    private void init() {
        try {
            RepHome repsDao = new RepHome();
            chkList = new ArrayList<JCheckBox>();
            reps = repsDao.getAllRep();
            for (Rep rep : reps) {
                chkList.add(new JCheckBox(rep.getName()));
            }
...
//            grousTable.getColumn(bundle.getString("delete")).setCellEditor(new ButtonEditor(this, new JCheckBox(), bundle));
    ...
    public void showRep() {
        ...
        Set<RolesReps> rrs  = role.getRolesRepses();
        clearAllChkBoxes();
        for (RolesReps rr : rrs) {
            //Roles role = (Roles) rolesDao.findByName(rg.getRoles().getRoleName());
            JCheckBox ckBox = new JCheckBox(rr.getRep().getName());
            ckBox.setSelected(true);
            int index = reps.indexOf(rr.getRep());
            ckBox.setSelected(true);
...
//        jScrollPane2 = new javax.swing.JScrollPane();
        ...
        jLabel2.setText(bundle.getString("reps")); // NOI18N
    ...
    private void addBtnActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        try {
            ...
            Set roleReps = new HashSet();
            for (int i = 0; i < chkList.size(); i++) {
                JCheckBox chk = chkList.get(i);
                if (chk.isSelected()) {
                    RolesReps rg = new RolesReps();
                    rg.setRoles(role);
                    Rep rep = reps.get(i);
                    rg.setRep(rep);
                    roleReps.add(rg);
